...
/**
 * A simple class to render (draw) a Slice of blocks, using the block's ImageComponent. It will draw a grid between the
 * blocks of a thickness of 1px (and 2 px all 5 blocks from the top left)
 * 
 * @author klaue
 */
public class ImageGrid extends JComponent implements Printable, MouseListener {
            ...
            for (int y = 0; y < vtBlockNum; ++y) {
                for (int x = 0; x < hzBlockNum; ++x) {
                    ...
                    if (this.markedBlock != null && this.markedBlock.x == x && this.markedBlock.y == y) {
                        ...
                        Color c = new Color(this.markColor.getRed(), this.markColor.getGreen(),
                                this.markColor.getBlue(), 50);
                        g.setColor(c);
    ...
    /**
     * This method writes the content of the current ImageGrid to a BufferedImage A check for enough memory should be
     * done before calling this or this should be wrapped inside a try-catch for java.lang.OutOfMemoryError - it'll need
     * ~4 byte of memory per pixel
     * 
    ...
    public Point getRelativeLocation(Point p) {
        ...
        if (p.x < igLocation.x || p.x > igLocation.x + this.getPreferredSize().width || p.y < igLocation.y
                || p.y > igLocation.y + this.getPreferredSize().height) {
            return null;
        }
    ...
    /**
     * gets the index of the block inside the slice. Yeah I know, using Point for this makes not much sense, but I'm not
     * gonna make a new Datatype just because Java lacks a generic return type for two values.
     * 
     * @param p
    ...
    private Point getIndexOfBlockAtPoint(Point p) {
        ...
        int xFiver = x / doubleLineXRepetitionRate; // the two lines at the end of each fiver-block count towards the
                                                    // previous fiver
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        this.printCache = new BufferedImage(this.getPreferredSize().width, this.getPreferredSize().height,
