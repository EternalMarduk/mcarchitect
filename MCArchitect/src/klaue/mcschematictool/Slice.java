...
public class Slice {
    ...
    public void cutOff(int left, int top, int right, int bottom) {
        ...
        if (left < 0 || top < 0 || right < 0 || bottom < 0 || left >= this.blocks.length || right >= this.blocks.length
            ...
            for (int i = 0; i < this.blocks.length; ++i) {
                for (int j = 0; j < this.blocks[i].length; ++j) {
                    if (this.blocks[i][j].isRedstoneWire()) {
                        ...
                        ((RedstoneWire) this.blocks[i][j])
                                .setWireType(wireInNorth, wireInEast, wireInSouth, wireInWest);
    ...
    /**
     * Exports the current slice as a BufferedImage. A check for enough memory should be done before calling this or
     * this should be wrapped inside a try-catch for java.lang.OutOfMemoryError because it'll need ~4 byte of memory per
     * pixel
     * 
     * @param zoom the current zoom value (1 is a 16x16 image/block)
