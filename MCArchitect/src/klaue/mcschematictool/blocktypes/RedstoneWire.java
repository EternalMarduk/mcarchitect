...
public class RedstoneWire extends Block {
    ...
    // first booelan is on (true), second is line (true) or "cross" (false). when not called by Slice, line defaults to
    // false and direction to NONE
    private static HashMap<Boolean, HashMap<Boolean, HashMap<Direction, BufferedImage>>> directionalWireImageCache = new HashMap<Boolean, HashMap<Boolean, HashMap<Direction, BufferedImage>>>();
    ...
    /**
     * Get the strength of the wire. Strength 0 is a non-powered one, strength 15 (0xF) is a wire right next to a power
     * source
     * 
     * @return the strength
     */
    ...
    /**
     * Set the strength of the wire. Strength 0 is a non-powered one, strength 15 (0xF) is a wire right next to a power
     * source
     * 
    ...
    public synchronized BufferedImage getImage(double zoom) {
        ...
        if (wireZoomCache != zoom) {
            ...
            HashMap<Boolean, HashMap<Direction, BufferedImage>> tempOnMap = directionalWireImageCache
                    .get(this.data != 0);
            if (tempOnMap != null) {
    ...
    /**
     * Sets the type of the wire. Since the direction of a redstone wire is not saved into the block data, this should
     * be called once the wires neighbors are known, see param direction
     * 
     * @param isLine true if this wire is a line, e.g. if the neighbors form a straight line
     * @param direction the direction. For non-line wires, this is the direction where the wire is in a 3x3 grid of
     *            wires. For example, a corner from bottom to the right would be in the top-left corner of the grid and
     *            therefore the direction would be north-west. A cross-like intersection would be in the middle, which
     *            is direction "none". For lines, this is the direction they are facing, a line from bottom to top would
     *            be north or south, a line from left to right would be west or east.<br>
     *            Note that this direction is not the direction inside the minecraft world but just used to determine
     *            the image. Therefore, north is always at the top of the 3x3 wire grid
     */
    public void setWireType(boolean isLine, Direction direction) {
        this.isLine = isLine;
        if (isLine) {
            if (direction != Direction.N && direction != Direction.E && direction != Direction.S
                    && direction != Direction.W) {
                throw new IllegalArgumentException("illegal direction for redstone wires that are lines " + direction);
            }
        } else if (direction == Direction.NNW || direction == Direction.NNE || direction == Direction.ENE
                || direction == Direction.ESE || direction == Direction.SSE || direction == Direction.SSW
                || direction == Direction.WSW || direction == Direction.WNW) {
    ...
    /**
     * Sets the type of the wire. Since the direction of a redstone wire is not saved into the block data, this should
     * be called once the wires neighbors are known, see boolean params
     * 
