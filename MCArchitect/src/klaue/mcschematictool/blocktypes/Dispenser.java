...
public class Dispenser extends DirectionalBlock {
    ...
    /**
     * An array of exactly 9 items corresponding to the 9 slots of a dispenser. Empty slots are Items with data = 0
     */
    public Item[] content = null;

    /**
     * Initializes the dispenser. The new dispenser will contain exactly 9 items, some or all of which may be zero
     * (empty)
     * 
    ...
    public String getToolTipText() {
        StringBuffer sb = new StringBuffer("<html><body>" + super.toString() + ", direction " + this.direction
                + ", contents:<br>");
        int i = 0;
        int j = 0;
    ...
    public JToolTip getCustomToolTip() {
        ...
        for (Item item : this.content) {
            ...
            if (amount > 1) {
                ...
                if (text.length == 2) {
                    g.drawImage(shadow[0], xPixels + 6, yPixels + 10, 8, 8, null, null); // overlaps 1px over the item
                                                                                         // icon frame
