...
public class MobHead extends DirectionalBlock {
    ...
    public enum HeadType {
        /** skeleton */
        SKELETON("Skeleton skull"), /** wither */
        WITHERSKELETON("Wither skeleton skull"), /** zombie */
        ZOMBIE("Zombie head"),
        /** human */
        HUMAN("Human head"), /** creeper */
     ...
     * @param data the blocks' normal data value, 1-5, determining if the head is free standing and if not, to which
     *            wall it's attached
     */
    ...
    public void setWallMounted(boolean isWallMounted) {
        if (this.direction == Direction.N || this.direction == Direction.E || this.direction == Direction.S
                || this.direction == Direction.W) {
            this.isWallMounted = isWallMounted;
        } else {
            throw new IllegalArgumentException("Head cannot be changed to wall mounted, illegal direction: "
                    + this.direction);
        }
    }
    ...
    public void turn(boolean CW) {
        if (this.isWallMounted) {
            ...
            /*
             * switch(this.direction) { case N: this.direction = CW ? Direction.NNE : Direction.NNW; break; case NNE:
             * this.direction = CW ? Direction.NE : Direction.N; break; case NE: this.direction = CW ? Direction.ENE :
             * Direction.NNE; break; case ENE: this.direction = CW ? Direction.E : Direction.NE; break; case E:
             * this.direction = CW ? Direction.ESE : Direction.ENE; break; case ESE: this.direction = CW ? Direction.SE
             * : Direction.E; break; case SE: this.direction = CW ? Direction.SSE : Direction.ESE; break; case SSE:
             * this.direction = CW ? Direction.S : Direction.SE; break; case S: this.direction = CW ? Direction.SSW :
             * Direction.SSE; break; case SSW: this.direction = CW ? Direction.SW : Direction.S; break; case SW:
             * this.direction = CW ? Direction.WSW : Direction.SSW; break; case WSW: this.direction = CW ? Direction.W :
             * Direction.SW; break; case W: this.direction = CW ? Direction.WNW : Direction.WSW; break; case WNW:
             * this.direction = CW ? Direction.NW : Direction.W; break; case NW: this.direction = CW ? Direction.NNW :
             * Direction.WNW; break; case NNW: this.direction = CW ? Direction.N : Direction.NW; break; default: //
             * should never happen throw new AssertionError(this.direction); }
             */
        }
    }
    ...
    public synchronized BufferedImage getImage(double zoom) {
        ...
        if (mobHeadZoomCache != zoom) {
            ...
            if (mobHeadImageCache.containsKey(this.headType)
                    && mobHeadImageCache.get(this.headType).containsKey(dirForCache)) {
