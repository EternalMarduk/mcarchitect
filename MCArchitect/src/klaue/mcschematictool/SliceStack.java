...
public class SliceStack implements Iterable<Slice> {
    ...
    public void addSlice(Slice slice) {
        ...
        if (!this.stack.isEmpty()) {
            if (this.stack.get(0).getWidth() != slice.getWidth() || this.stack.get(0).getHeight() != slice.getHeight()) {
    ...
    /**
     * Removes "Whitespace", eg air blocks, from all sides of the stack, so that the resulting stack is the smallest
     * cuboid without removing any non-air-blocks
     */
    public void trim() {
    ...
    public void cutOff(int top, int bottom, int north, int east, int south, int west) {
        ...
        if (top < 0 || bottom < 0 || north < 0 || east < 0 || south < 0 || west < 0 || top >= this.stack.size()
                || bottom >= this.stack.size() || west >= this.getSlice(0).getWidth()
                || east >= this.getSlice(0).getWidth() || north >= this.getSlice(0).getHeight()
                || south >= this.getSlice(0).getHeight() || (top + bottom) >= this.stack.size()
     ...
     * Note that the images in the grids will be updated if the blocks change, so you have to call this or
     * Slice.getImages() or Block.getImage() for every change
     * 
                    ...
                    for (int j = 0; j < slice.getHeight(); ++j) {
                        if (slice.getBlockAt(i, j).isRedstoneWire()) {
                            ...
                            if (j != 0) {
                                ...
                                wireInNorth = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                        || b.isPressurePlate() || b.isButton() || b.isDetectorRail() || b.isRepeater() || b
                                        .isFenceGate());
                            }
                            if (slice.getWidth() - 1 != i) {
                                Block b = slice.getBlockAt(i + 1, j);
                                wireInEast = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                        || b.isPressurePlate() || b.isButton() || b.isDetectorRail() || b.isRepeater() || b
                                        .isFenceGate());
                            }
                            if (slice.getHeight() - 1 != j) {
                                Block b = slice.getBlockAt(i, j + 1);
                                wireInSouth = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                        || b.isPressurePlate() || b.isButton() || b.isDetectorRail() || b.isRepeater() || b
                                        .isFenceGate());
                            }
                            if (i != 0) {
                                Block b = slice.getBlockAt(i - 1, j);
                                wireInWest = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                        || b.isPressurePlate() || b.isButton() || b.isDetectorRail() || b.isRepeater() || b
                                        .isFenceGate());
                            }

                            // upper level (only if there is a upper level and the block above is air)
                            if ((idx != this.stack.size() - 1)
                                    && !blockBlocksWire(this.stack.get(idx + 1).getBlockAt(i, j))) {
                                Slice upperSlice = this.stack.get(idx + 1);
                                if (j != 0 && !wireInNorth) {
                                    Block b = upperSlice.getBlockAt(i, j - 1);
                                    wireInNorth = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                            || b.isPressurePlate() || b.isDetectorRail() || b.isRepeater() || b
                                            .isFenceGate());
                                }
                            ...
                            if ((idx != 0)) {
                                ...
                                if (j != 0 && !wireInNorth && !blockBlocksWire(slice.getBlockAt(i, j - 1))) {
                                    ...
                                    wireInNorth = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                            || b.isPressurePlate() || b.isButton() || b.isDetectorRail()
                                            || b.isRepeater() || b.isFenceGate());
                                }
                                if (slice.getWidth() - 1 != i && !wireInEast
                                        && !blockBlocksWire(slice.getBlockAt(i + 1, j))) {
                                ...
                                if (slice.getHeight() - 1 != j && !wireInSouth
                                        && !blockBlocksWire(slice.getBlockAt(i, j + 1))) {
                                    Block b = lowerSlice.getBlockAt(i, j + 1);
                                    wireInSouth = (b.isRedstoneWire() || b.isRedstoneTorch() || b.isLever()
                                            || b.isPressurePlate() || b.isButton() || b.isDetectorRail()
                                            || b.isRepeater() || b.isFenceGate());
                            ...
                            ((RedstoneWire) slice.getBlockAt(i, j)).setWireType(wireInNorth, wireInEast, wireInSouth,
                                    wireInWest);
                            ...
                            ((TripWire) slice.getBlockAt(i, j)).setWireType(wireInNorth, wireInEast, wireInSouth,
                                    wireInWest);
                        }
                    }
                }
                imgGrid[idx] = this.stack.get(idx).getImages(zoom, false); // false for redstone calc because that was
                                                                           // allready done
