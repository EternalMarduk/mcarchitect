...
/**
 * This class reads the minecraft images and provides them trough getters and setters This whole class is static and
...
public class ImageProvider {
    ...
    /**
     * Gets the image with the specified name. Make sure not to change the returned reference (only copies of it) or the
     * provider will get compromised
     * 
    ...
    /**
     * Gets the image with the specified name from the additional image set. Make sure not to change the returned
     * reference (only copies of it) or the provider will get compromised
     * 
    ...
    /**
     * Gets the image with the specified name from the item image set. Make sure not to change the returned reference
     * (only copies of it) or the provider will get compromised
     * 
     ...
     * @return 8x8px letter-images for the input string. note that minecraft cuts off the last two pixels (which are
    ...
    /**
     * Rotates the given image clockwise by the given angle. A new image instance will be returned, the original image
     * will not be changed.
     * 
    ...
    public static BufferedImage rotateImage(double degrees, final BufferedImage img) {
        ...
        Graphics2D g = newImg.createGraphics();
        AffineTransform at = AffineTransform.getRotateInstance(Math.toRadians(degrees), img.getWidth() / 2,
                img.getHeight() / 2);
        g.drawRenderedImage(img, at);
        return newImg;
    }
    ...
    /**
     * Uses multiply to blend this image with the given color (same effect as a layer filled with a color set to
     * multiply in GIMP) A new image instance will be returned, the original image will not be changed.
     * 
    ...
    /**
     * Returns an image representing the given ID. Note that only images on zoom level 1 are returned, and that
     * additional overlays, like the direction, are not added. The image returned is exactly as the one in minecrafts
     * terrain.png or item.png with the exception of stairs and slabs/double slabs (there is no stair image in the
     * files, it's just a normal block with one edge cut out, and all slabs beside stone slabs also just use the normal
     * block image cut in half). So if you, for example, need an image of a rail turned the right way to accomondate
     * it's direction, or a torch that has an arrow added for the direction, get an instance of the block by
     * Block.getInstance() and use it's getImage method. Same as a call to getImageByBlockOrItemID(<id>, -1);
     * 
     * @param id
     ...
     * Returns an image representing the given ID. Note that only images on zoom level 1 are returned, and that
     * additional overlays, like the direction, are not added. The image returned is exactly as the one in minecrafts
     * texture directory with the exception of stairs and slabs/double slabs (there is no stair image in the files, it's
     * just a normal block with one edge cut out, and all slabs beside stone slabs also just use the normal block image
     * cut in half).
     * 
     * @param id
    ...
    public final static synchronized BufferedImage getImageByBlockOrItemID(short id, byte data) {
        switch (id) {
            ...
            case 44:// stone slab
                // all double slabs beside stone look just like the regular blocks and the slabs like half of the
                // regular blocks
            ...
            case 156: // quartz
                ...
                // it makes no sense to do that here so for this block, we use the blocks getImage functionality (data 0
                // = direction E)
            ...
            case 373:
            case 374:
                return ImageProvider.getItemImage("glassBottle"); // Potion and glass bottle (color of potions is added
                                                                  // in ItemPotion)
            case 375:
    ...
    private static TreeMap<String, BufferedImage> getImagesFromJarDir(ClassLoader classLoader, String dirPath)
     ...
     * @throws IOException
     */
    private static String[] getResourceListing(ClassLoader classLoader, String path) throws URISyntaxException,
            IOException {
        ...
        if (dirURL == null) {
            /*
             * In case of a jar file, we can't actually find a directory. Have to assume the same jar as this.
             */
