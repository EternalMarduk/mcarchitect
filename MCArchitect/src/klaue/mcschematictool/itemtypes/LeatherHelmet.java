...
public class LeatherHelmet extends ColoredItem {
    ...
    public synchronized BufferedImage getImage(double zoom) {
        // since we plan to use the parents getImage and use its imagecache too (no need to waste space storing colored,
        // but incomplete leather armor parts)
        // we have to check if the image is in the cache before we call the parents getImage (or else it would be there
        // anyway but no one could tell if it was
        ...
        if (coloredZoomCache != zoom) {
            ...
            if (coloredZoomCache == zoom && coloredItemCache.containsKey(this.id)
                    && coloredItemCache.get(this.id).containsKey(this.data)
        ...
        // image was not in cache - get parents colored image and add overlay (we make no new image - new image in cache
        // is updated)
